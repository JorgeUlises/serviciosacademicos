...
class SqlgestionRecuperarClave extends sql {
	...
	function cadena_sql($tipo,$variable="") {
		...
		switch($tipo) {
			...
			case "conteoEstudiante":
				$cadena_sql = "SELECT COUNT(*) FROM ACEST WHERE EST_COD = ".$variable." ";
				break;
				
			case "updateGeclaves":
				$cadena_sql = " update geclaves SET CLA_PROCESADO='N' WHERE cla_codigo in (".$variable.") ";
				break; 
				
			case "agregarRegistro":
				$cadena_sql = "INSERT INTO REGISTRO_RECUPERACION_CLAVE ";
				$cadena_sql .= " (ID_REGISTRO_REC , ESTUDIANTE, USUARIO) ";
                $cadena_sql .= " VALUES ";
                $cadena_sql .= " ((SELECT NVL(MAX(ID_REGISTRO_REC),0) + 1 FROM REGISTRO_RECUPERACION_CLAVE), ";
