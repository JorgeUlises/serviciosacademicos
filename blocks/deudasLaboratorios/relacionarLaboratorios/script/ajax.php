...
$cadenaACodificar1=$cadenaACodificar."&funcion=obtenerLista";
...
<script type='text/javascript'>
	...
	function consultarSeleccionUsuario(el, opt){
			var id = el.id;
			id = id.replace('sel','');
	...
	function editarElemento(id,el,idLab,idEst){
	    ...
	    for(i=0;i<row.cells.length;i++){ 
		    ...
		    if(string.indexOf('name="listadoEstados"')==-1&&string.indexOf('name="listadoLaboratorios"')==-1
		    		&&string.indexOf('name="Ano"')==-1&&string.indexOf('name="Periodo"')==-1){
				...
				$.ajax({
		            ...
		            success: function(jresp){
		            	if(jresp[0]==true){
					       ...
					       }else alert("<?php echo utf8_encode($lenguaje->getCadena("errorOperacion"));?>");
				       }
		        });
			}
		}
	...
	function agregarFila(){ 
		    ...
		    $clone.attr('style', '' );
		    $clone.find(':text').val('');
		    $('#tablaEdicion tr:last').after($clone);
